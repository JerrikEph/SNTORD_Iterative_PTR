...
def reduce_avg(reduce_target, lengths, dim):
    ...
    if rank_diff!=0:
        len_shape = tf.concat(0, [tf.shape(lengths), [1]*rank_diff])
        mask_shape = tf.concat(0, [tf.shape(mask), [1]*rank_diff])
    else:
    ...
    red_sum = tf.reduce_sum(mask_target, reduction_indices=[dim], keep_dims=False)
...
def self_attn(inputs, inputs_lengths):
    ...
    attn_matrix_1 = tf.expand_dims(attn_matrix_1, dim=2)    # shape(b_sz, tstp, 1, attn_sz)
    attn_matrix_2 = tf.expand_dims(attn_matrix_2, dim=1)    # shape(b_sz, 1, tstp, attn_sz)
    ...
    attn_logits = tf.select(attn_mask, attn_logits, tf.ones_like(attn_logits)*small_num)
    ...
    attn_content = tf.reduce_sum(attn_input * attn_prob,
                                 reduction_indices=[2])     # shape(b_sz, tstp, rep_sz)
    return attn_content


def last_dim_linear(inputs, output_size, bias, bias_start=0.0, scope=None):
    '''
    Args:
    ...
    out_shape = tf.concat(0, [input_shape[:-1], [output_size]])
    input_size = np.int(inputs.get_shape()[-1])
    unbatch_input = tf.reshape(inputs, shape=[-1, input_size])

    unbatch_output = tf.nn.rnn_cell._linear(unbatch_input, output_size, bias=bias,
