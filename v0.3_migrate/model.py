    ...
    def train_module(self, cell_dec, encoder_inputs,
        ...
        def enc(dec_h, in_x, lengths, fake_call=False):
            ...
            def func_f(in_x, enc_h, in_h_hat, fake_call=False):
                ...
                enc_in = tf.tile(enc_in_ex,  # shape(b_sz, tstp_dec, tstp_enc, s_emb_sz)
                                 [1, tstp_dec, 1, 1])
                res = tf.concat(axis=3, values=[enc_in, enc_h])
                return res      # shape(b_sz, tstp_dec, tstp_enc, enc_emb_sz+h_enc_sz*2)
            ...
            lstm_out, _ = tf.nn.bidirectional_dynamic_rnn(cell_enc,
                                                          ...
                                                          dtype=tf.float32,
                                                          scope='sent_encoder')
            enc_out = tf.concat(axis=2, values=lstm_out)    # shape(b_sz*tstp_dec, tstp_enc, h_enc_sz*2)
            enc_out = tf.reshape(enc_out,       # shape(b_sz, tstp_dec, tstp_enc, h_enc_sz*2)
                                 shape=[b_sz, tstp_dec,
                                        tstp_enc, h_enc_sz*2])

            res = func_f(in_x, enc_out, None)
        ...
        def func_enc_input(dec_h, enc_input, fake_call=False):
            ...
            enc_in_tile = tf.tile(enc_in_ex,            # shape(b_sz, tstp_dec, tstp_enc, s_emb_sz)
                                  [1, tstp_dec, 1, 1])
            output = tf.concat(axis=3,                       # shape(b_sz, tstp_dec, tstp_enc, s_emb_sz+h_dec_sz)
        ...
        def func_point_logits(dec_h, enc_ptr, enc_len):
            ...
            Args:
                ...
                enc_len : shape(b_sz,)
            '''
            dec_h_ex = tf.expand_dims(dec_h, axis=2)     # shape(b_sz, tstp_dec, 1, h_dec_sz)
            dec_h_ex = tf.tile(dec_h_ex, [1, 1, tstp_enc, 1])  # shape(b_sz, tstp_dec, tstp_enc, h_dec_sz)
            linear_concat = tf.concat(axis=3, values=[dec_h_ex, enc_ptr])  # shape(b_sz, tstp_dec, tstp_enc, h_dec_sz+ Ptr_sz)
            ...
            point_logits = tf.squeeze(point_v, axis=[3])  # shape(b_sz, tstp_dec, tstp_enc)

            enc_len = tf.expand_dims(enc_len, 1)                # shape(b_sz, 1)
            enc_len = tf.tile(enc_len, [1, tstp_dec])           # shape(b_sz, tstp_dec)
            mask = TfUtils.mkMask(enc_len, maxLen=tstp_enc)     # shape(b_sz, tstp_dec, tstp_enc)
            point_logits = tf.where(mask, point_logits,        # shape(b_sz, tstp_dec, tstp_enc)
        ...
        def get_initial_state(hidden_sz):
            ...
            avg_in_x = TfUtils.reduce_avg(encoder_inputs,           # shape(b_sz, s_emb_sz)
                                          enc_lengths, dim=1)
            state = TfUtils.linear(avg_in_x, hidden_sz,     # shape(b_sz, hidden_sz)
                                           bias=False,
        ...
        def decoder():
            def get_dec_in():
                ...
                dec_in = tf.concat(axis=1, values=[bos, dec_in])                                # shape(b_sz, tstp_dec+1, s_emb_sz)
    ...
    def decoder_test(self, cell_dec, encoder_inputs,
        ...
        def enc(dec_h, in_x, lengths, fake_call=False):
            ...
            def func_f(in_x, in_h, in_h_hat, fake_call=False):
                ...
                if not in_h_sz:
                    assert ValueError('last dimension of the second' +
                                      ' arg should be known, while got %s'
                                      % (str(type(in_h_sz))))
                res = tf.concat(axis=2, values=[in_x, in_h])
                return res
            ...
            lstm_out, _ = tf.nn.bidirectional_dynamic_rnn(cell_enc,
                                                          ...
                                                          dtype=tf.float32,
                                                          scope='sent_encoder')
            enc_out = tf.concat(axis=2, values=lstm_out)  # shape(b_sz, tstp_enc, h_enc_sz*2)
        ...
        def func_enc_input(dec_h, enc_input, fake_call=False):
            ...
            output = tf.concat(axis=2, values=[enc_input, dec_h_tile])  # shape(b_sz, tstp_enc, s_emb_sz + h_dec_sz)
        ...
        def func_point_logits(dec_h, enc_e, enc_len):
            ...
            dec_h_ex = tf.expand_dims(dec_h, axis=1)  # shape(b_sz, 1, h_dec_sz)
            dec_h_ex = tf.tile(dec_h_ex, [1, tstp_enc, 1])  # shape(b_sz, tstp_enc, h_dec_sz)
            linear_concat = tf.concat(axis=2, values=[dec_h_ex, enc_e])  # shape(b_sz, tstp_enc, h_dec_sz+ dec_emb_sz)
            ...
            point_v = TfUtils.last_dim_linear(  # shape(b_sz, tstp_enc, 1)
                tf.tanh(point_linear), output_size=1,
                bias=False, scope='Ptr_V')
            point_logits = tf.squeeze(point_v, axis=[2])  # shape(b_sz, tstp_enc)
        ...
        def func_point_idx(dec_h, enc_e, enc_len, hit_mask):
            ...
            prob = tf.where(hit_mask, tf.zeros_like(prob),
    ...
    def fetch_input(self):
        ...
        def lstm_sentence_rep(input):
            with tf.variable_scope('lstm_sentence_rep_scope') as scope:
                ...
                rep_out = tf.concat(axis=2, values=rep_out) #(b_sz*tstps_en, len_sen, h_sz*2)
        ...
        def cnn_sentence_rep(input):
            ...
            def convolution(input, tstps_en, length):
                ...
                input = tf.concat(axis=2, values=conv_outputs) #(b_sz*tstps_en, len_sen, out_channel * len(filter_sizes))
