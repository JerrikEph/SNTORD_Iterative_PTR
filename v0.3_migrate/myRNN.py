    ...
    **NOTE: This method is still in testing, and the API may change.**
    ...
    with vs.variable_scope(scope or "RNN") as varscope:
        ...
        (elements_finished, next_input,
         initial_state, hit_mask, emit_structure) = loop_fn(
            time, None, None, None)  # time, cell_output, cell_state
        flat_input = nest.flatten(next_input)

        # Need a surrogate loop state for the while_loop if none is available.
        loop_state = constant_op.constant(0, dtype=dtypes.int32)
        ...
        def body(time, elements_finished, current_input, emit_ta, state, hit_mask):
            ...
            (next_finished, next_input,
             next_state, next_hit_mask, emit_output) = loop_fn(
                next_time, next_output, cell_state, hit_mask)
            ...
            return (next_time, elements_finished,
                    next_input, emit_ta, next_state, next_hit_mask)

        returned = control_flow_ops.while_loop(
            condition, body, loop_vars=[
                time, elements_finished, next_input, emit_ta, state, hit_mask],
            parallel_iterations=parallel_iterations, swap_memory=swap_memory)

        (emit_ta, final_state, _) = returned[-3:]
