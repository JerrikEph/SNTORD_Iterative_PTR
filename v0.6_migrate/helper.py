...
def batch_encodeNpad(data, label, vocab):
    ...
    ret_label = [[i[j] if j<len(i) else 0 for j in range(max_sent_num)] for i in label]
...
"""Prediction """
def calculate_accuracy_seq(pred_matrix, label_matrix, lengths):
    """
    Args:
    ...
    def seq_equal(seq_a, seq_b, length):
        for i in range(length):
            if seq_a[i] != seq_b[i]:
                return False
        return True
    print len(lengths), len(label_matrix), '=='*30
    match = [seq_equal(pred_matrix[i], label_matrix[i], lengths[i]) for i in range(len(label_matrix))]
